...
- **חיווי כן למשתמש** (`SaveStatus`): "✓ נשמר" / "שומר… (3)" /
  "אין חיבור · 3 ממתינים". בלי זה התור בלתי נראה ואפשר לצאת מיום שלם שלא נשלח.
- **סמנטיקה**: התור מוצא כפילויות לפי (יום, ילד) — רק הסימון האחרון נשמר, אז
  תיקון לא מצטבר מאחורי הקודם. הסרה אחרי הצלחה היא לפי `queuedAt` ולא לפי ילד,
  אחרת תיקון שנעשה בזמן שהבקשה באוויר היה נמחק בשקט (יש בדיקה בדיוק לזה).
- **כשל קבוע מול כשל זמני**: 4xx (הילד כבר לא באירוע) מוסר מהתור ומדווח; רשת
  או 5xx נשמרים לניסיון חוזר. אחרת פריט אחד תקוע היה חוסם את כל מה שאחריו.
- **זיכרון הוא מקור האמת**, ו-localStorage הוא העמידות בין טעינות. בגלישה
  פרטית שבה הכתיבה נחסמת הסימונים עדיין נשלחים.
- ריקון התור: באירוע `online`, ובנוסף poll כל 15 שניות — `navigator.onLine`
  משקר מספיק (רשתות שבויות, דאטה סלולרי מקרטע) כדי שלא נסמוך עליו לבד.
- אזהרה בסגירת טאב אם נשארו סימונים בתור.
- **PWA**: `src/app/manifest.ts` + אייקונים + `public/sw.js` בכתיבה ידנית (בלי
  ספרייה — service worker הוא sticky, ולכן חייב להיות קצר מספיק כדי לקרוא
  במלואו). network-first עם נפילה ל-cache; **לא** מטמון ל-`/api/` כי נוכחות
  חייבת להיות עדכנית.
